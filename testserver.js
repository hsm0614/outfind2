...
app.post('/signup/contractor', (req, res) => {
    ...
    const { businessNumber, email, password, industry, subIndustry, location } = req.body;
    ...
    const sql = `INSERT INTO contractors (business_number, email, password, industry, sub_industry, location) VALUES (?, ?, ?, ?, ?, ?)`;
    connection.query(sql, [businessNumber, email, password, industry, subIndustry, location], (error, results, fields) => {
        if (error) {
            console.error('MySQL 데이터베이스에 회원 정보 삽입 중 오류 발생:', error);
            res.status(500).send('회원가입 중 오류가 발생했습니다.');
            return;
    ...
    connection.query(query, [companyEmail, companyPassword], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            const token = jwt.sign({ userId: results[0].id }, secretKey);
                // 토큰을 클라이언트에게 반환
                res.json({ token: token, userType: 'company' });
...
app.post("/login/contractor", function(req, res) {
    ...
    connection.query(query, [contractorEmail, contractorPassword], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            const token = jwt.sign({ userId: results[0].id }, secretKey);
                // 토큰을 클라이언트에게 반환
                res.json({ token: token, userType: 'contractor' });
        } else {
            res.status(401).send("이메일 또는 비밀번호가 올바르지 않습니다."); // 로그인 실패 시 실패 메시지를 클라이언트로 보냄
        }
    });
